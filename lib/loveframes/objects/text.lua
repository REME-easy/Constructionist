...
function newobject:SetText(t)
	...
	for k, v in ipairs(tdata) do
		...
		elseif dtype == "string" then
			...
			v = loveframes.utf8.gsub(v, "\n", " \n ")
			v = loveframes.utf8.gsub(v, "%[.-].-%[/.-]", function(pattern) return " " .. pattern .. " " end)
	...
	for k, v in ipairs(textdata) do
		...
		local color = v.color
		if detectlinks then
			if loveframes.utf8.len(text) > 5 and loveframes.utf8.find(text, "%[.-]") then
				v = hep.format.regex(v)
				text = v.text
				color = v.color
			end
		end
...
function newobject:DrawText()
	...
	local printfunc = function(text, x, y)
		love.graphics.print(text, math.floor(x + 0.5), math.floor(y + 0.5), x)
	end
