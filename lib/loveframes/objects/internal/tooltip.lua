...
function newobject:update(dt)
	...
	if object then
		...
		if ohover then
			...
			-- modified
			if self.x + textobject.width > love.graphics.getWidth() then
				self.x = self.x - textobject.width
			end
			if self.y + textobject.height > love.graphics.getHeight() then
				self.y = self.y - textobject.width
			end
			--
			if not top then
				self:MoveToTop()
